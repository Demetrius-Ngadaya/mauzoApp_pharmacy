...
<script type="text/javascript">
    ...
    if (avai_qty === "" || in_qty > avai_qty_int || in_qty <= 0) {
        ...
        setTimeout(function() {
            alert("Haujachagua Dawa au umeandika idadi unayotaka kuhamisha idayozidi idadi iliyopo !!");
        }, 100);
    } else {
...
<?php include('indexSideBar.php') ?>
  ...
  <div class="content-wrapper">
    ...
    <a href="hamisha_kwa_mkopo.php?invoice_number=<?php echo $_GET['invoice_number']?>" style="color: white; text-decoration: none;">Rifresh</a>
</button>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <center>
                <div class="col-md-12">
       <form method="POST" action="insert_invoice_hamisha_kwa_mkopo.php?invoice_number=<?php echo $_GET['invoice_number']?> " style="display: flex; flex-wrap: wrap; gap: 10px;">
        ...
        <div class="form-group">
            <input type="number" name="qty" id="qty" placeholder="Idadi unayohamisha" autocomplete="off" class="form-control" style="width: 160px; height: 30px;" required>
        </div>
        
        
        <!-- Fourth div for 'Punguzo' -->
        <div class="form-group">
            <input type="number" name="discount" id="Punguzo" placeholder="Bei ya kuuzia" class="form-control" style="width: 140px; height: 30px;">
        </div>
        
            <!-- <input  name="hali_ya_malipo" id="hali_ya_malipo" value="credit"  style="width: 0px; height: 0px;" required>
        
        <div class="form-group">
        <select  name="payment_method" required> 
          <option value="Credit">Mode of payment</option>
        ...
        <!-- <div class="form-group">
            <input type="text" name="hali_ya_malipo" id="hali_ya_malipo" placeholder="status" autocomplete="off" class="form-control" style="width: 190px; height: 30px;" required>
        </div> -->
        <div class="form-group">
    <input type="tel" name="phone_number" id="phone_number" placeholder="Simu ya mteja (07...)" 
           class="form-control" style="width: 190px; height: 30px;" required
           pattern="0[0-9]{9}" title="Namba ya simu lazima ianze na 0 na kuwa na tarakimu 10">
    <small class="text-muted">Lazima ianze na 0 na kuwa na tarakimu 10</small>
</div>
        <div class="form-group">
          <!-- <label for="">Tarehe atakayolipa</label> -->
            <input type="date" name="date_to_pay" id="date_to_pay"  autocomplete="off" class="form-control" style="width: 160px; height: 30px;" required>
        </div>
      ...
      <table class="table table-bordered table-striped table-hover" id="resultTable" data-responsive="table">
        ...
        <thead>
          <tr>
           ...
           <td><a href="delete_invoice_hamisha_kwa_mkopo.php?invoice_number=<?php echo $_GET['invoice_number']?>&id=<?php echo $row['id'];?>&name=<?php echo $row['medicine_name']?>&expire_date=<?php echo $row['expire_date']?>&quantity=<?php echo $row['qty'];?>" class="btn btn-danger">Ondoa</a></td>
              ...
              <td  colspan="3"><strong>
                ...
                while($row = mysqli_fetch_array($select_query)){
                  ...
                  $grand_profit = number_format($row['sum(profit_amount)']);
          ...
          <?php
           if($medicine_name && $category && $quantity !=null){
            ...
            <a id="popup" href="checkout_hamisha_kwa_mkopo.php?invoice_number=<?php echo $_GET['invoice_number']?>&medicine_name=<?php echo $medicine_name?>&category=<?php echo $category?>&ex_date=<?php echo $ex_date?>&quantity=<?php echo $quantity?>&total=<?php echo $grand_total?>&profit=<?php echo $grand_profit?>" style="width:400px;" class="btn btn-info btn-large">Jina la mteja</i></a>
