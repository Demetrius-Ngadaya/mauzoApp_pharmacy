...
<?php include('indexSideBar.php') ?>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title"><b>Orodha ya Dawa zilizo nunuliwa kwa mkopo</b></h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                        ...
                                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                            <thead>
                                                <tr class="table-success">
                                                    ...
                                                    <th>Dawa</th>
                                                    <th>Aina ya Dawa</th>
                                                    <th>Idadi iliyokopwa</th>
                                                    <th>Namba ya risiti</th>
                                                    <!-- <th>Namba ya bachi</th>   -->
                                                    <th>Tarehe iliyopokelewa</th>    
    ...
    $select_sql = "SELECT * FROM purchases_report WHERE store_id = '$store_id' and status='credit'";
    ...
    if (mysqli_num_rows($select_query) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($select_query)) {
            ...
            if ($row['status'] == 'credit') {
                echo '<button type="button" class="btn btn-sm btn-success" onclick="confirmMarkAsPaid(' . $row['id'] . ');">Ifanye imelipwa</button>';
            } else {
                echo "Paid";
            }
            echo "</td>";
        ...
        echo '<tr><td colspan="11">Hauna Dawa ambazo umenunua kwa mkopo </td></tr>';
    }
    ?>
<script>
    ...
    function confirmMarkAsPaid(saleId) {
        ...
        Swal.fire({
            title: 'Je unauhakika?',
            text: "Unakaribia kuifanya Dawa imelipwa!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Ndio, na uhakika!'
        }).then((result) => {
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = 'update_credit_status.php';  // Update the correct form action
