                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-header">
                                <h3 class="card-title"><b>Dawa zilizouzwa kwa mkopo  </b></h3>
                            </div>
...
$select_sql = "SELECT sales.*, stores.location  
FROM sales
JOIN stores ON sales.store_id = stores.id
WHERE hali_ya_malipo='not paid'
AND sales.store_id = '$store_id'";
...
if (mysqli_num_rows($select_query) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($select_query)) {
        ...
        if ($row['hali_ya_malipo'] == 'not paid') {
            echo '<button type="button" class="btn btn-sm btn-success" onclick="confirmMarkAsPaid(' . $row['id'] . ');">Iweke amelipa</button>';
        } else {
            echo "Paid";
        }
        echo "</td>";
    ...
    echo '<tr><td colspan="12">Katika mauzo yako hakuna Dawa ya mkopo </td></tr>';
}
?>
<script>
    ...
    function confirmMarkAsPaid(saleId) {
        ...
        Swal.fire({
            title: 'Je unauhakika?',
            text: "Unakaribia kubadili taharifa ya mauzo kuwa yamishalipiwa!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Ndio , amelipa!'
        }).then((result) => {
