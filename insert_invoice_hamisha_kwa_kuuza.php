...
$date_to_pay = $_POST['date_to_pay'] ?? null;
...
$profit_amt = (($discounted_price - $actual_price) *$qty);
        ...
        $insert_sql = "INSERT INTO on_hold (invoice_number, medicine_name, category, expire_date, qty,type, cost, amount, discount, payment_method, phone_number, date_to_pay, profit_amount, date, hali_ya_malipo, store_id, transfer_status) 
                       VALUES ('$invoice_number', '$medicine_name', '$category', '$expire_date', '$qty','$sell_type','$cost', '$amount', '$discounted_price', '$payment_method', '$phone_number', '$date_to_pay', '$profit_amt', '$date', '$status', '$store_id', '$transfer_status')";

        $insert_query = mysqli_query($con, $insert_sql);

	if($insert_query){

     header("location:hamisha_kwa_kuuza.php?invoice_number=$invoice_number");
  
     // echo "<script type='text/javascript'>window.location.href = hamisha_kwa_kuuza.php?invoice_number=$invoice_number '</script>";
	}else{
