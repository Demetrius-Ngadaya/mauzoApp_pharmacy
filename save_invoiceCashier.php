    ...
    if (mysqli_num_rows($select_on_hold_query) > 0) {
        while ($row = mysqli_fetch_array($select_on_hold_query)) {
            ...
            $insert_sql = "INSERT INTO sales (
                ...
                payment_method, phone_number, date_to_pay, customer_name, created_by, date, hali_ya_malipo,store_id
            ) VALUES (
                '$medicine_id', '$invoice_number', '$medicine_name', '$category', '$quantity', '$amount', '$profit_amount', 
                '$payment_method', " . ($phone_number ? "'$phone_number'" : "NULL") . ", " . ($date_to_pay ? "'$date_to_pay'" : "NULL") . ", '$paid_amount', '$created_by', '$date', '$status', '$store_id'
            )";
